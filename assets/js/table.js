...
// TODO: Consider adding a responsive mobile-friendly weekly calendar portrait view feature
function generateCalendar (eventData) {
    var eventData = eventData.data.sort(function(a, b) { return (new Date(a.startdate)) - (new Date(b.startdate)) })
...
function appendEvent( event ) {
    ...
    if (event.url.includes("events.html#sutd") || event.url.includes("events.html#fic") || event.url.includes("events.html#housing-sessions") ) {
        var eventElement   = $('<div class="event" style="background-color:lightcoral;"><a target="_blank" href="' + event.url + '">' + event.name + '</a></div>')
            ...
            var eventElement   = $('<div class="event"><a>' + event.name + '</a></div>')
                ...
                var eventElement   = $('<div class="event"><a target="_blank" href="' + event.url + '">' + event.name + '</a></div>')
                }
    // Handle multi-days
    if ( eventEndDate.getDate() ) {
        ...
        while ( eventEndDate > date ) {
            ...
            dateElement.removeClass('no-event').append('<div class="event multi-days following-days" title="' + event.name + '"><a target="_blank" href="' + event.url + '">' + event.name + '</a></div>')
        }
    }
...
function generateMonthTable( date ) {
    ...
    monthTable.before('<h6 style="font-size:1px;color:white;" data-month="' + date.getFullYear() + '-' + eventMonthName + '">' + eventMonthName + ' ' + date.getFullYear() + '</h6>')
